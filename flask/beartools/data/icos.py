...
class Fetch:
    ...
    def check_data_swap_status(self):
        ...
        """
        print('ICOS: Fetch | data swap status | ... checking data availability in data swap storage ...')
        ...
        print(f'ICOS: Fetch | data swap status | {self.ID}: {station_status},  {self.param}: {param_status}')
        return {'station': station_status, 'param': param_status}

    def fetch_station_metadata(self):
        print('ICOS: Fetch | station metadata | ... fetching data infos, including dobj (download link) ...')
        ...
        print(f'ICOS: Fetch | station metadata | Fetching metadata for {self.ID} successful.')
        return meta
    ...
    def fetch_data(self):
        print(f'ICOS: Fetch | data | ... fetching data ...')
    ...
    def fetch_and_swap_data(self, data_swap):
        if all(self.data_swap_status.values()):
            print('ICOS: Fetch | data | Skip fetching, data already swapped.')

        elif not self.data_swap_status['station']:
            data = self.fetch_data()
            print(f'ICOS: Fetch | data | swaping data, creating entry:  {self.ID} with {self.param}')
